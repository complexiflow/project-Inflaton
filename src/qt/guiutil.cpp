...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("inflaton"))
        return false;
...
{
    // Convert inflaton:// to inflaton:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("inflaton://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "inflaton:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "inflaton.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for inflaton.lnk
...
{
    return GetAutostartDir() / "inflaton.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a inflaton.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=inflaton\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the inflaton app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add inflaton app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("inflaton-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  inflaton-qt [" + tr("command-line options") + "]                     " + "\n";
