...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeinflatonAmount"))
        settings.setValue("nAnonymizeinflatonAmount", 1000);
    nAnonymizeinflatonAmount = settings.value("nAnonymizeinflatonAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeinflatonAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeinflatonAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeinflatonAmount:
            return QVariant(nAnonymizeinflatonAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeinflatonAmount:
            nAnonymizeinflatonAmount = value.toInt();
            settings.setValue("nAnonymizeinflatonAmount", nAnonymizeinflatonAmount);
            emit AnonymizeinflatonAmountChanged(nAnonymizeinflatonAmount);
            break;
        default:
            break;
        }
