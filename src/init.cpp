...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: inflaton.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: inflatond.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeinflatonamount=<n> " + _("Keep N inflaton anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. inflaton is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. inflaton is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("inflaton version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "inflaton server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of inflaton") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart inflaton to complete") << "\n";
    ...
    nAnonymizeinflatonAmount = GetArg("-anonymizeinflatonamount", 0);
    if(nAnonymizeinflatonAmount > 999999) nAnonymizeinflatonAmount = 999999;
    if(nAnonymizeinflatonAmount < 2) nAnonymizeinflatonAmount = 2;
    ...
    LogPrintf("Anonymize inflaton Amount %d\n", nAnonymizeinflatonAmount);
